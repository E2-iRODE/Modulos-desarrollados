...
class AgedPartnerBalanceCustomHandlerInherited(models.AbstractModel):
    _inherit = 'account.aged.partner.balance.report.handler'

    def _prepare_partner_values(self):
        vals = super()._prepare_partner_values()
        vals.update({
            'provider_invoice_number': '',
            'provider_invoice_serial': '',
            'fel_gt_dte_number': '',
            'fel_gt_serie': '',
        })
        return vals
    ...
    def _aged_partner_report_custom_engine_common(
        ...
        is_receivable = internal_type == 'asset_receivable'
        ...
        def build_result_dict(report, query_res_lines):
            ...
            if current_groupby == 'id':
                ...
                currency = self.env['res.currency'].browse(query_res['currency_id'][0]) \
                    if len(query_res['currency_id']) == 1 else None
                expected_date = (
                    len(query_res['expected_date']) == 1 and query_res['expected_date'][0]
                    or len(query_res['due_date']) == 1 and query_res['due_date'][0]
                )

                if is_receivable:
                    dte_numbers = [v for v in (query_res.get('fel_gt_dte_number') or []) if v]
                    dte_series  = [v for v in (query_res.get('fel_gt_serie')       or []) if v]
                    extra = {
                        'fel_gt_dte_number': dte_numbers[0] if dte_numbers else '',
                        'fel_gt_serie':      dte_series[0]  if dte_series  else '',
                    }
                else:
                    inv_numbers = [v for v in (query_res.get('provider_invoice_number') or []) if v]
                    inv_serials = [v for v in (query_res.get('provider_invoice_serial')  or []) if v]
                    extra = {
                        'provider_invoice_number': inv_numbers[0] if inv_numbers else '',
                        'provider_invoice_serial':  inv_serials[0]  if inv_serials  else '',
                    }
                ...
                extra = (
                    {'fel_gt_dte_number': '', 'fel_gt_serie': ''}
                    if is_receivable else
                    {'provider_invoice_number': '', 'provider_invoice_serial': ''}
                )
                rslt.update({
                    'invoice_date':    None,
                    'due_date':        None,
        ...
        if is_receivable:
            extra_select = """
                ARRAY_AGG(DISTINCT move.fel_gt_dte_number) AS fel_gt_dte_number,
                ARRAY_AGG(DISTINCT move.fel_gt_serie)      AS fel_gt_serie,
            """
        else:
            extra_select = """
                ARRAY_AGG(DISTINCT move.provider_invoice_number) AS provider_invoice_number,
                ARRAY_AGG(DISTINCT move.provider_invoice_serial)  AS provider_invoice_serial,
            """
